...
from model.VisAwareTranslation import (postprocessing, translate_sentence,
...
class ncNet(object):
    def __init__(self, trained_model):
        ...
        SRC_VECTORS = SRC.vocab.vectors.to(device)

        enc = Encoder(SRC_VECTORS, HID_DIM, ENC_LAYERS, ENC_HEADS, ENC_PF_DIM, ENC_DROPOUT,
                      self.device, self.TOK_TYPES, self.my_max_length)
        ...
        self.ncNet = Seq2Seq(enc, dec, self.SRC, SRC_PAD_IDX, TRG_PAD_IDX,
                             self.device).to(self.device)  # define the transformer-based ncNet
    ...
    def specify_dataset(self, data_type, db_url=None, table_name=None, data=None, data_url=None):
        ...
        if data_type == 'csv':
            ...
            if data == None and data_url != None:
                ...
                raise ValueError(
                    'Read JSON from the json file, please only specify the "data_type" or "data_url"'
                )
            ...
            try:
                ...
                raise ValueError(
                    'Errors in read table from sqlite3 database. \ndb_url: {0}\n table_name : {1} '.
                    format(data_url, table_name))
        ...
        self.db_tables_columns = {self.db_id: {self.table_id: list(self.data.columns)}}
        ...
        if data_type == 'json' or data_type == 'sqlite3':
            ...
            conn = sqlite3.connect('./dataset/database/' + self.db_id + '/' + self.db_id +
                                   '.sqlite')
        ...
        for table, cols in table_cols.items():
            col_val_map = self.DataProcesser.get_values_in_columns(self.db_id,
                                                                   table,
                                                                   cols,
                                                                   conditions='remove')
    ...
    def nl2vis(self,
        ...
        if visualization_aware_translation == True:
            ...
            pred_query, attention, enc_attention = translate_sentence_with_guidance(
                self.db_id, self.table_id, input_src, self.SRC, self.TRG, self.TOK_TYPES,
                token_types, self.SRC, self.ncNet, self.db_tables_columns,
                self.db_tables_columns_types, self.device, self.my_max_length, show_progress)
            ...
            return VegaLite(query2vl.to_VegaLite(pred_query, self.data)), query2vl.to_VegaLite(
                pred_query, self.data)
            # print('\n')

        else:
            # print("\nGenerate the visualization by greedy decoding:\n")

            pred_query, attention, enc_attention = translate_sentence(input_src, self.SRC, self.TRG,
                                                                      self.TOK_TYPES, token_types,
                                                                      self.ncNet, self.device,
                                                                      self.my_max_length)
    ...
    def process_input(self, nl_question, chart_template):
        ...
        def fix_chart_template(chart_template=None):
            ...
            if chart_template != None:
                ...
                try:
                    if 'sorting_options' in chart_template and chart_template[
                        ...
                        if 'type' in chart_template['sorting_options']:
                            if chart_template['sorting_options']['type'].lower() == 'desc':
                                ...
                                raise ValueError(
                                    'Unknown order by settings, the order-type must be "desc", or "asc"'
                                )
                        query_template = query_template.replace(
                            'sort [S]', 'sort ' + order_xy + ' ' + order_type)
                except:
        ...
        input_src = "<N> {} </N> <C> {} </C> <D> {} <COL> {} </COL> <VAL> {} </VAL> </D>".format(
            nl_question, query_template, self.table_id, col_names, value_names).lower()
...
if __name__ == '__main__':
    ncNet = ncNet(trained_model='./save_models/trained_model.pt')
    ncNet.specify_dataset(data_type='sqlite3',
